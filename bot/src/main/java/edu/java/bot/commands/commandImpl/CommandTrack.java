...
public class CommandTrack implements CallableCommand {

    private final Reminder reminder;

    @Autowired
    public CommandTrack(Reminder reminder) {
    ...
    public SendMessage callableHandle(Update update, Printer printer) throws MalformedURLException {
        try {
            reminder.addUrl(update.message().chat().id(), new URL(update.message().text()));
        ...
        return printer.getMessage(update.message()
                                        .chat()
                                        .id(), "url is successfully tracked");
    }
}
