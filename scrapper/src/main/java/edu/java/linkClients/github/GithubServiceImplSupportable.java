...
public class GithubServiceImplSupportable implements GithubServiceSupportable {
    ...
    @Override public RepositoryResponse getGithubRepository(@NotNull String owner, @NotNull String repo) {
    ...
    @Override public List<IssueResponse> getListComments(@NotNull String owner, @NotNull String repo) {
        return webClient.get().uri("/repos/{owner}/{repo}/issues/comments", owner, repo).retrieve()
                        .bodyToFlux(IssueResponse.class).onErrorResume(WebClientResponseException.class, Flux::error)
                        .collectList().block();
    }

    @Override public List<PullRequestResponse> getListPullRequests(@NotNull String owner, @NotNull String repo) {
        return webClient.get().uri("/repos/{owner}/{repo}/pulls", owner, repo).retrieve()
                        .bodyToFlux(PullRequestResponse.class)
                        .onErrorResume(WebClientResponseException.class, Flux::error).collectList().block();
    ...
    @Override public Optional<LinkUpdateResponse> getLastUpdateDate(String pathOfUrl, OffsetDateTime lastUpdate) {
        ...
        List<PullRequestResponse> pullRequestResponseMono = getListPullRequests(owner, repo).stream().filter(
            pullRequestResponse -> pullRequestResponse.createdAt().isAfter(lastUpdate)).toList();
        List<IssueResponse> listComments = getListComments(
            owner,
            repo
        ).stream().filter(issueResponse -> issueResponse.createdAt()
                                                        .isAfter(lastUpdate))
         .toList();
        ...
        if (githubRepository.updatedAt().isAfter(lastUpdate)) {
            ...
            descriptionBuilder.append(repositoryBeginning).append(githubRepository.name()).append(" was changed\n");
        }
        if (!pullRequestResponseMono.isEmpty()) {
            for (PullRequestResponse pullRequestResponse : pullRequestResponseMono) {
                ...
                descriptionBuilder.append(repositoryBeginning).append(githubRepository.name())
        ...
        if (!listComments.isEmpty()) {
            for (IssueResponse listComment : listComments) {
                ...
                descriptionBuilder.append(repositoryBeginning).append(githubRepository.name())
                                  ...
                                  .append("link to commit: ").append(listComment.htmlUtl()).append("\n");
            }
        }
