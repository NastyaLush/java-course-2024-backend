...
public class JdbcUrlUpdater implements UrlUpdater {
        ...
        getNotCheckedForLingTimeUrls().forEach(
                urlEntity -> {
                    ...
                    try {
                        ...
                        OffsetDateTime lastUpdateDate = linkService.get()
                                                                   .getLastUpdateDate(uri.getPath());
    ...
    private void update(OffsetDateTime lastUpdateDate, UrlEntity urlEntity, URI uri) {
        if (lastUpdateDate.isAfter(urlEntity.lastUpdate())) {
            ...
            List<Long> tgChatIdsWithThisUrl = urlService.getChats(urlEntity.id())
                                                        .stream()
                                                        .map(
                                                                ChatEntity::tgChatId)
                                                        .toList();
            LinkUpdate linkUpdate =
                    new LinkUpdate().id(urlEntity.id())
                                    .url(uri)
